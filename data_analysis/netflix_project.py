...
# 세선 저장소에 업로드한 csv 파일을 읽어 변수에 할당
...
# 시청 등급 코드를 더 이해하기 쉬은 표현으로 매핑한 딧셔너리 정의(key, value 선언)
age_group_dic = {
    'G' : 'All',
    'TV-G' : 'All',
    'TV-Y' : 'All',
...
#map 함ㅅ를 이용하여 rating 컬럼의 값을 딕셔너리를 기반으로 변환하여 age_group 컬럼에 저장
#.map( ) : 사정의 정의한 내용을 변수에 적용
...
plt.title('We see more movies tha TV shows onn Netflix.', fontfamily = 'serif', fontsize=12)
plt.show()
